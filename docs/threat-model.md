...
Security claims require rejection-path evidence. Unit tests cover configuration rejection, allowlist resolution, ticket expiry/reuse/identity binding, protocol parsing, frontend stale-event and ticket lifecycle, bounded UTF-8 input chunking, state transitions, read-only input handling, and limit enforcement. Integration tests cover wrong-Origin requests, unticketed WebSockets, real-browser identity/ticket/WebSocket/PTY flow, secret-free URLs, PTY lifecycle and resize, child teardown, bounded output, and the production proxy, audit, and SSH controls when their later chunks land.
...
- **Pre-release immaturity.** Interfaces and assumptions may change. M1's local terminal controls are implemented, but production authentication, transport gating, rate limiting, audit persistence, SSH execution, and deployment hardening remain incomplete. The current build must not be deployed as a terminal gateway.
